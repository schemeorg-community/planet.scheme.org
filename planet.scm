...
(define (sxml-find sxml . path)
  (let loop ((node sxml) (path path))
    (if (null? path)
        node
        (let ((child (find (lambda (x) (and (pair? x) (eq? (car x) (car path))))
                           (if (pair? node) (cdr node) '()))))
          (and child (loop child (cdr path)))))))
  ...
  (let* ((feed (sxml-find sxml 'atom:feed))
	  ...
	  (or (let ((link-node
                     (find (lambda (x)
                             (and (pair? x)
                                  (eq? (car x) 'atom:link)
                                  (let ((rel (sxml:attr x 'rel)))
                                    (or (not rel) (string=? rel "alternate")))
                                  (let ((type (sxml:attr x 'type)))
                                    (or (not type)
              ...
              (let ((self-link
                     (and feed
                          (find (lambda (x)
                                  (and (pair? x)
                                       (eq? (car x) 'atom:link)
                                       (equal? (sxml:attr x 'rel) "self")))
                                (cdr feed)))))
                (and self-link (sxml:attr self-link 'href)))
              ""))
